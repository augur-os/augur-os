...
This module validates that all skills in the plugins/ directory follow
...
# Required version.yaml fields (current schema baseline)
REQUIRED_VERSION_YAML_FIELDS = ["version"]
RECOMMENDED_VERSION_YAML_FIELDS = ["updated", "created", "skill", "name", "codename", "status"]
VALID_STATUS_VALUES = ["stable", "beta", "alpha", "deprecated", "active"]
...
def get_all_skill_dirs() -> list[Path]:
    """Discover strict-compliance skill directories for test parameterization."""
    ...
    def has_required_file(skill_dir: Path, filename: str) -> bool:
        return (skill_dir / filename).exists() or (skill_dir / "_dev" / filename).exists()

    def has_version_file(skill_dir: Path) -> bool:
        return (
            (skill_dir / "augur" / "version.yaml").exists()
            or has_required_file(skill_dir, "version.yaml")
            or (skill_dir / "pyproject.toml").exists()
        )

    def is_strict_compliance_candidate(skill_dir: Path) -> bool:
        return (
            has_required_file(skill_dir, "SKILL.md")
            and has_required_file(skill_dir, "README.md")
            and has_required_file(skill_dir, "CHANGELOG.md")
            and has_version_file(skill_dir)
        )
    ...
    # Legacy pre-skills layout: plugins/{vertical,horizontal,factory}/{skill}
    for layer_name in ("vertical", "horizontal", "factory"):
        layer_dir = plugins_dir / layer_name
        ...
        for skill_dir in layer_dir.iterdir():
            ...
            if resolved in seen or not is_strict_compliance_candidate(skill_dir):
                continue
            seen.add(resolved)
            skill_dirs.append(skill_dir)

    # Legacy bundled plugin layout: plugins/{bundle}/skills/{skill}
...
class TestSkillRequiredFiles:
    """Test that skill plugins have required files per CONTRIBUTING.md."""
    ...
    def test_skill_md_exists(self, skill_dir: Path):
        ...
        if skill_dir.name == "augur-mcp":
            pytest.skip("augur-mcp is a platform server, not a standard skill")
    ...
    def test_version_yaml_exists(self, skill_dir: Path):
        ...
        augur_version_file = skill_dir / "augur" / "version.yaml"
        version_file = skill_dir / "version.yaml"
        dev_version_file = skill_dir / "_dev" / "version.yaml"
        pyproject_file = skill_dir / "pyproject.toml"

        exists = (
            augur_version_file.exists() or version_file.exists() or dev_version_file.exists() or pyproject_file.exists()
        )

        assert exists, (
            f"Missing required augur/version.yaml (or pyproject.toml) in {skill_dir.name}. "
...
class TestSkillMdFrontmatter:
    ...
    def test_has_name_field(self, skill_dir: Path):
        ...
        assert "name" in frontmatter, f"SKILL.md in {skill_dir.name} must have 'name' field in frontmatter"
        assert frontmatter["name"], f"SKILL.md 'name' field in {skill_dir.name} must not be empty"
    ...
    def test_has_description_field(self, skill_dir: Path):
        ...
        assert frontmatter["description"], f"SKILL.md 'description' field in {skill_dir.name} must not be empty"
...
class TestSkillMdSections:
    ...
    def test_has_capabilities_section(self, skill_dir: Path):
        ...
        capability_keywords = ("capabilities", "key capabilities", "overview", "what this does", "features")
...
class TestVersionYaml:
    ...
    def test_version_yaml_has_updated(self, skill_dir: Path):
        """version.yaml SHOULD have lifecycle timestamp metadata."""
        version_file = skill_dir / "augur" / "version.yaml"
        if not version_file.exists():
            version_file = skill_dir / "version.yaml"
        if not version_file.exists():
        ...
        if "updated" not in data and "created" not in data:
            pytest.skip("No updated/created timestamp field (optional in current schema)")

    @pytest.mark.parametrize("skill_dir", SKILL_DIRS, ids=SKILL_IDS)
    def test_version_yaml_has_skill(self, skill_dir: Path):
        """version.yaml MUST include an identity field ('skill' or 'name')."""
        version_file = skill_dir / "augur" / "version.yaml"
        if not version_file.exists():
            version_file = skill_dir / "version.yaml"
        if not version_file.exists():
        ...
        assert "skill" in data or "name" in data, (
            f"version.yaml in {skill_dir.name} must include 'skill' or 'name'. "
            f"Found keys: {sorted(list(data.keys())) if isinstance(data, dict) else '<invalid>'}"
        )
...
class TestNamingConventions:
    ...
    def test_version_yaml_skill_matches_directory(self, skill_dir: Path):
        """version.yaml skill/name identifier SHOULD be kebab-case and directory-aligned."""
        version_file = skill_dir / "augur" / "version.yaml"
        if not version_file.exists():
            version_file = skill_dir / "version.yaml"
        if not version_file.exists():
        ...
        kebab_pattern = r"^[a-z][a-z0-9]*(-[a-z0-9]+)*$"
        assert re.match(
            kebab_pattern, str(skill_name)
        ), f"version.yaml 'skill' field ({skill_name}) should be kebab-case"

        # Allow bundle-level directories that expose a canonical sub-skill identifier.
        if skill_name != skill_dir.name and data.get("name") != skill_dir.name:
            pytest.skip("skill identifier maps to a canonical sub-skill, not directory name")
...
class TestReadmeCompliance:
    ...
    def test_readme_has_title(self, skill_dir: Path):
        ...
        assert len(h1_headers) >= 1, f"README.md in {skill_dir.name} should have an H1 title"
...
class TestSkillSummary:
    ...
    def test_at_least_one_skill_exists(self):
        ...
        if not SKILL_DIRS:
            pytest.skip("No skills found in repo (skills may be managed in augur)")
        assert len(SKILL_DIRS) > 0, "No skills found in plugins/ directory"

    def test_all_skills_have_skill_md(self):
        """All skills MUST have SKILL.md."""
        if not SKILL_DIRS:
            pytest.skip("No skills found in repo (skills may be managed in augur)")
        missing = [d.name for d in SKILL_DIRS if d.name != "augur-mcp" and not (d / "SKILL.md").exists()]
    ...
    def test_skill_count_summary(self):
        ...
        print(f"\n  Found {count} skills in plugins/")
        # Count compliant skills
        compliant = sum(
            1 for d in SKILL_DIRS if all((d / f).exists() or (d / "_dev" / f).exists() for f in REQUIRED_FILES)
        )
