...
- Audit timestamp: 2026-02-11T14:14:00.997555
...
You are implementing **ADR-067: Career Hub Hardening**.

Read the full ADR: `docs/decisions/ADR-067-career-hardening.md`
...
1. **Create team**: `TeamCreate(team_name="adr-067-career-hardening", description="Implementing ADR-067: Career Hub Hardening")`
2. **Create tasks**: For each step in the Execution Plan, create a task via `TaskCreate`. Set `blocked_by` for PIPELINE dependencies.
3. **Spawn teammates**: For each unique agent role, spawn a teammate:
   ```
   Task(subagent_type="general-purpose", team_name="adr-067-career-hardening", name="{role}",
...
**Team name**: `adr-067-career-hardening`
...
- [ ] ADR-067 status updated to Accepted

## User Notes

- **Focus on real data flow**: Priority is replacing mock/hardcoded data with real YAML data fetching across all pages. 7/15 pages currently use hardcoded arrays instead of fetching from `plugins/consulting/career/`.
- **High-priority hub**: Career hub is the most-used hub and should be demo-ready first. Treat this as the flagship hardening effort.
