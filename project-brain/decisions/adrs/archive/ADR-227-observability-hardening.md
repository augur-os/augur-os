...
|---|-----------|-------|--------|--------|-------------|
| 1 | UI Compliance | 99/100 | 12% | good | - |
| 2 | Page Coverage | 100/100 | 10% | good | - |
| 3 | API Completeness | 100/100 | 12% | good | - |
| 4 | MCP Tool Wiring | 93/100 | 10% | good | 8 actions use API-wrapped MCP pattern |
...
| 7 | Workflows | 90/100 | 8% | good | 8/8 actions are functionally executable (2 autonomous, 6 ... |
...
Automated hardening audit of **Observability** (http://localhost:3000/observability?tab=self-heal) on 2026-03-04.
...
- Audit timestamp: 2026-03-04T10:07:28.807594
...
Read the full ADR: `docs/decisions/ADR-227-observability-hardening.md`
...
1. **Create team**: `TeamCreate(team_name="adr-204-observability-hardening", description="Implementing ADR-227: Observability Hardening")`
2. **Create tasks**: For each step in the Execution Plan, create a task via `TaskCreate`. Set `blocked_by` for PIPELINE dependencies.
3. **Spawn teammates**: For each unique agent role, spawn a teammate:
   ```
   Task(subagent_type="general-purpose", team_name="adr-204-observability-hardening", name="{role}",
...
**Team name**: `adr-204-observability-hardening`
...
| V.2 | frontend | low | Browser validation: open http://localhost:3000/observability?tab=self-heal in Chrome MCP, screenshot each tab, check console for runtime errors, verify auth gates render cleanly |
