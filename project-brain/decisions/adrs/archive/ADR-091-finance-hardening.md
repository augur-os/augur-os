...
| 6 | User Value | 25/100 | 15% | critical | 6 real data files found in finance/ data dir |
| 7 | Workflows | 70/100 | 8% | needs-work | 3/3 actions have working backends |
| 8 | Cross-Hub Connectivity | 0/100 | 5% | critical | No pages to check — hub is isolated |
| 9 | Action Buttons | 100/100 | 8% | good | 3/3 actions are fully-wired |
| 10 | Wow Effect | 60/100 | 10% | needs-work | Best candidate: Live Balance Sheet |

**Composite Score**: 22/100 (major-rebuild)

## Wow Effect: Live Balance Sheet

> Real-time net worth dashboard reading directly from Balance-Sheet.xlsx via Python/openpyxl API

**Score**: 60/100

**Demo Flow**:
1. User opens Finance Dashboard
2. Balance Sheet (Live) section loads automatically from Excel file
3. 6 stat cards show net worth, assets, loans, cash, income, USD equivalent
4. User clicks Refresh to pull latest data from the spreadsheet
5. Data updates in real-time without page reload

**Current state**: Live Balance Sheet already functional (ADR-090)
**Gap to demo-ready**: Already implemented via ADR-090 — needs page coverage and cross-hub wiring for full score

**Other candidates**:
- Add Transaction (20/100, modal_workflow)
- Analyze Spending (20/100, llm_action)
- Tax Strategy (20/100, llm_action)

**Priority**: This is the first thing to implement in Phase 1.

## Context

Automated hardening audit of **Finance** (http://localhost:3000/finance) on 2026-02-12.
...
**User Value** (25/100):
- 6 real data files found in finance/ data dir
...
**Wow Effect** (60/100):
- Best candidate: Live Balance Sheet
- Description: Real-time net worth dashboard reading directly from Balance-Sheet.xlsx
- Gap to demo-ready: Already functional — needs page coverage and cross-hub connectivity for full score
...
- 3/3 actions have working backends
...
- Killer demo use case identified: Live Balance Sheet
...
- Audit timestamp: 2026-02-12T23:40:28.307471

## User Notes

ADR-090 import work is already completed on main branch (uncommitted). This includes:
Live Balance Sheet with API route and Python script, 3 new tabs (Calculators, Documents, Knowledge),
Notion knowledge migration, and connections.yaml. The audit ran against the worktree which
lacks these changes, so scores are artificially low. The hardening ADR should focus on
remaining gaps: missing page.tsx files for existing tabs (Accounts, Transactions, Budget,
Portfolio, Crypto, Goals, Retirement, Taxes), API completeness, MCP tool wiring,
cross-hub connectivity, and polishing the wow effect.
...
You are implementing **ADR-091: Finance Hardening**.

Read the full ADR: `docs/decisions/ADR-091-finance-hardening.md`
...
1. **Create team**: `TeamCreate(team_name="adr-088-finance-hardening", description="Implementing ADR-091: Finance Hardening")`
2. **Create tasks**: For each step in the Execution Plan, create a task via `TaskCreate`. Set `blocked_by` for PIPELINE dependencies.
3. **Spawn teammates**: For each unique agent role, spawn a teammate:
   ```
   Task(subagent_type="general-purpose", team_name="adr-088-finance-hardening", name="{role}",
...
**Team name**: `adr-088-finance-hardening`
...
|------|-------|------|------|-------|
| 1.1 | developer | high | Fix Wow Effect (60/100): Best candidate: Live Balance Sheet | `plugins/finance/skills/finance/augur.yaml`, `plugins/finance/skills/finance/augur/` |
...
| 1.7 | architect | high | Fix User Value (25/100): 6 real data files found in finance/ data dir | `plugins/finance/skills/finance/augur.yaml` |
...
| Step | Agent | Tier | Task | Files |
|------|-------|------|------|-------|
| 3.9 | developer | medium | Fix Workflows (70/100): 3/3 actions have working backends | `plugins/finance/skills/finance/augur.yaml` | Chains: `generate_delight` |
...
- [ ] ADR-091 status updated to Accepted
