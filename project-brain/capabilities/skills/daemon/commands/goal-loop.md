...
2. **Create the worktree:** run `aug routine goal-worktree <goal-id> --stamp <utc-stamp>`.
...
3. **Drain the backlog first** (the original motivation): run
   `aug routine goal-drain-backlog --loops <comma-loops>`. It returns `entries`
   ...
   `aug routine goal-consume-finding --entry-id <id>` so it is cleared and never
...
4. **Per loop, in the returned order** (test/build before hygiene), iterate:
   a. `aug routine goal-scan-loop --loop <loop> --worktree <path> --budget-used <n>
   ...
   b2. For each `maintenance` item, do **NOT** spawn a subagent — it is a deterministic
      ...
      `aug routine goal-run-maintenance --loop <loop> --worktree <path>
   ...
   c. After each bucket's subagent returns, run `aug routine goal-record-bucket
   ...
   d. Re-run `goal-scan-loop`; pass the previously saved and current
      `residual_fingerprint` to `aug routine goal-loop-status --prev-fingerprint
      ...
      - `stalled` or `exhausted` → escalate the residual findings with
        `aug routine goal-escalate --findings-json '<json>'`, then go to the next loop.
