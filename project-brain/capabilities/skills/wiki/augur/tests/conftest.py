"""Shared test bootstrap for wiki skill tests.
...
- add wiki skill scripts/ to sys.path so `from <module>` works for tests
  that load wiki internals
"""
...
# Make `skills.wiki.*` and `skills.ingest.*` dotted imports resolvable.
...
_WIKI_SCRIPTS = Path(__file__).resolve().parents[2] / "scripts"
