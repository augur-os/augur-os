---
description: Project-scoped stale-version artifact sweep. Use through /project sweep.
visibility: project
x-augur-export-command: false
x-augur-parent-command: project
x-augur-project-verb: sweep
---
...
```bash
/project sweep <path>                  # dry-run (default)
/project sweep <path> --apply          # destructive
/project sweep <path> --paths-only     # emit only the paths that would be archived; no reasoning text
```

For `/project sweep`, `<path>` is a folder under `Au-docs/` (absolute or relative). Browse Sources, Notes, and Pages use typed selection MCP tools instead of this path argument. The `sweep-stores` capability is an internal terminal implementation detail, not the user-facing command surface.
