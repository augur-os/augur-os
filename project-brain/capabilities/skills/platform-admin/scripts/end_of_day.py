        ...
        if log_dir.exists():
            for log_file in log_dir.glob("**/*.log"):
                try:
                    ...
                    logger.debug("Failed to parse log file %s: %s", log_file, exc)
    ...
    for inbox_path in inbox_paths:
        if inbox_path.exists():
            try:
                ...
                logger.debug("Failed to parse inbox file %s: %s", inbox_path, exc)
    ...
    if backlog_dir.exists():
        for task_file in backlog_dir.glob("**/*.md"):
            ...
            try:
                ...
                logger.debug("Failed to parse backlog task %s: %s", task_file, exc)
