...
def load_dependencies(repo_root: Path) -> dict:
    ...
    deps_file = repo_root / "platform-admin" / "data" / "dependencies.yaml"
    if not deps_file.exists():
        _out(f"Error: {deps_file} not found")
        sys.exit(1)
...
def get_git_last_modified(filepath: Path, repo_root: Path) -> Optional[str]:
    ...
    try:
        ...
        result = _run_command(
            ["git", "log", "-1", "--format=%ci", "--", str(rel_path)], cwd=repo_root, capture_output=True, text=True
        )
...
def get_git_last_commit(filepath: Path, repo_root: Path) -> Optional[str]:
    ...
    try:
        rel_path = filepath.relative_to(repo_root)
        result = _run_command(
            ["git", "log", "-1", "--format=%h", "--", str(rel_path)], cwd=repo_root, capture_output=True, text=True
        )
        if result.returncode == 0 and result.stdout.strip():
            return result.stdout.strip()
    except Exception:
...
class DependencyGraph:
    ...
    def get_file_state(self, path: str) -> dict:
        ...
        # Handle glob patterns (e.g., plugins/career-consultant/modules/)
        if path.endswith("/"):
    ...
    def print_status(self):
        ...
        _out("\n" + "=" * 60)
        _out(f"SUMMARY: {needs_update}/{total} files need attention")
        _out("=" * 60)

    def print_graph(self):
        """Print the dependency graph in ASCII format."""
        _out("=" * 60)
        _out("DEPENDENCY GRAPH")
        _out("=" * 60)
        ...
        def print_tree(path, prefix="", is_last=True):
            ...
            _out(f"{prefix}{connector}[{node_type}] {path}")
    ...
    def print_affected(self, path: str):
        ...
        _out(f"Files affected by changes to: {path}")
        _out("-" * 40)
    ...
    def print_rebuild_order(self):
        ...
        for i, (path, reasons) in enumerate(rebuild_list, 1):
            node = self.get_node(path)
            _out(f"{i}. {path}")
            _out(f"   Type: {node.get('type', 'unknown')}")
            _out(f"   Reason: {', '.join(reasons)}")

            if node.get("rebuild_instructions"):
                instructions = node["rebuild_instructions"].strip().split("\n")[0]
                _out(f"   Instructions: {instructions}...")
            _out()
...
def main():
    ...
    parser.add_argument(
        "command", choices=["status", "check", "graph", "rebuild-order", "affected", "prompt"], help="Command to run"
    )
    parser.add_argument("file", nargs="?", help="File path (for check and affected commands)")
    ...
    if args.command == "status":
        ...
        if not args.file:
            _out("Error: file path required for check command")
            sys.exit(1)
        needs, reasons = graph.check_needs_update(args.file)
        if needs:
            _out(f"NEEDS UPDATE: {args.file}")
            _out(f"Reasons: {', '.join(reasons)}")
        else:
            _out(f"UP TO DATE: {args.file}")
        ...
        if not args.file:
            _out("Error: file path required for affected command")
            sys.exit(1)
        graph.print_affected(args.file)

    elif args.command == "prompt":
        _out(graph.generate_claude_prompt())
