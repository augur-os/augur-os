"""auto-todo-cleanup: Find and resolve TODO_CLEANUP markers.

Extracted from CodeQualityLoop._run_todo_fix for todo-cleanup category (ADR-200).
"""
...
def scan(ctx: OpsContext) -> ScanResult:
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            issues = [
                {"action": "todo-cleanup", "file": f, "marker": "TODO_CLEANUP"}
                for f in files[:20]  # Cap at 20 files per scan
            ]
            return ScanResult(
                issues=issues,
                summary=f"Found TODO_CLEANUP markers in {len(files)} files",
                severity="warning",
            )
        return ScanResult(
            issues=[],
            summary="No TODO_CLEANUP markers found",
            severity="info",
        )
...
def fix(ctx: OpsContext, issues: list[dict]) -> FixResult:
    """Use headless Claude to resolve TODO_CLEANUP markers."""
    if ctx.dry_run:
        return FixResult(
            success=True,
            summary=f"Dry run: would fix {len(issues)} TODO_CLEANUP markers",
        )
    ...
    for issue in issues:
        ...
        prompt = (
            f"Find and fix the TODO_CLEANUP marker in: {file_path}. "
    ...
    return FixResult(
        ...
        actions=all_actions,
        changes=all_changes,
        summary=f"Fixed {len(all_changes)} TODO_CLEANUP markers",
    )
