"""Tests for auto-fix scan/fix protocol."""
...
_MODULE_PATH = Path(__file__).resolve().parents[2] / "scripts" / "ops" / "auto_fix.py"
_SPEC = importlib.util.spec_from_file_location("auto_fix_under_test", _MODULE_PATH)
...
def test_module_name() -> None:
    assert mod.name == "auto-fix"


def test_scan_returns_empty(tmp_path: Path) -> None:
    """Scan is a no-op shim that always returns empty issues."""
...
def test_fix_dry_run(tmp_path: Path) -> None:
    result = mod.fix(_ctx(tmp_path, dry_run=True), [{"marker": "TODO_BUG"}])
...
def test_fix_no_issues(tmp_path: Path) -> None:
    ...
    assert result.success is True
    assert "No marker issues" in result.summary


def test_fix_queues_issues(tmp_path: Path) -> None:
    result = mod.fix(_ctx(tmp_path), [{"marker": "a"}, {"marker": "b"}])
    assert result.success is True
