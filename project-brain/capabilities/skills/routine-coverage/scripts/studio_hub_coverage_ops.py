"""auto-studio-hub-coverage: repair stale legacy path references in studio docs."""
...
name = "auto-studio-hub-coverage"

DIFFICULTY_SPEC = {
    0: "Surface scan — find stale legacy skill paths in studio markdown",
    1: "Content check — resolve replacements against current skill roots",
    2: "Deep check — rewrite current-hub markdown references in-place",
}

_LEGACY_SKILL_PATH_RE = re.compile(r"plugins/([a-z-]+)/skills/([a-z0-9-]+)(/[^\s`'\"<)]*)?")
...
def scan(ctx: OpsContext) -> ScanResult:
    issues: list[dict] = []
    for md_file in _iter_studio_markdown_files(ctx.project_root):
        try:
        ...
        issues.append(make_issue(
            ...
            path=rel_path,
            detail=(
                f"{md_file.name} still references legacy plugin skill paths. "
                f"Next: rewrite {len(replacements)} reference(s) to the live `project-brain/capabilities/skills/...` layout."
            ),
    ...
    if not issues:
        return ScanResult(
            issues=[],
            summary="Studio hub markdown references are aligned with the live skill layout",
            severity="info",
            health="verified",
        )
    ...
    return ScanResult(
        issues=issues,
        summary=f"Found {len(issues)} studio markdown file(s) with stale legacy skill paths",
        severity="warning",
    )
...
def fix(ctx: OpsContext, issues: list[dict]) -> FixResult:
    if ctx.dry_run:
        return FixResult(success=True, summary=f"Dry run: {len(issues)} studio doc fix(es)")
    ...
    if not changes:
        return FixResult(success=True, summary="No studio hub reference changes were required", fix_type="verified")

    return FixResult(
        success=True,
        actions=actions,
        changes=changes,
        summary=f"Repaired stale studio hub references in {len(changes)} markdown file(s)",
        fix_type="code-fix",
    )
