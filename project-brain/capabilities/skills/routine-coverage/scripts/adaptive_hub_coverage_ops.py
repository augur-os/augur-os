"""auto-adaptive-hub-coverage: repair stale legacy references in adaptive docs."""
...
name = "auto-adaptive-hub-coverage"

DIFFICULTY_SPEC = {
    0: "Surface scan — find stale adaptive-hub references",
    1: "Content check — resolve replacements against live skill and workflow paths",
    2: "Deep check — rewrite stale adaptive-hub references in-place",
}
...
_EXACT_REPLACEMENTS = {
    "skills/developer/chains/auto-fix-markers.yaml": "project-brain/capabilities/skills/devops/scripts/ops/auto_fix.py",
    "skills/ai/augur/data/agent-workflows/test-nightly.md": "project-brain/capabilities/skills/validator/commands/test-nightly.md",
}
...
def _iter_adaptive_markdown_files(project_root: Path) -> list[Path]:
    files: list[Path] = []
    for skill_md in sorted((project_root / "project-brain" / "capabilities" / "skills").glob("*/SKILL.md")):
        fm = _read_frontmatter(skill_md)
        if fm.get("x-augur-hub") != "adaptive":
            continue
...
def scan(ctx: OpsContext) -> ScanResult:
    issues: list[dict] = []
    for md_file in _iter_adaptive_markdown_files(ctx.project_root):
        try:
        ...
        issues.append(make_issue(
            ...
            path=rel_path,
            detail=(
                f"{md_file.name} still references stale adaptive-hub skill or workflow paths. "
                f"Next: rewrite {len(replacements)} reference(s) to live client-native locations."
            ),
    ...
    if not issues:
        return ScanResult(
            issues=[],
            summary="Adaptive hub markdown references are aligned with live skill and workflow locations",
            severity="info",
            health="verified",
        )
    ...
    return ScanResult(
        issues=issues,
        summary=f"Found {len(issues)} adaptive markdown file(s) with stale references",
        severity="warning",
    )
...
def fix(ctx: OpsContext, issues: list[dict]) -> FixResult:
    if ctx.dry_run:
        return FixResult(success=True, summary=f"Dry run: {len(issues)} adaptive doc fix(es)")
    ...
    if not changes:
        return FixResult(success=True, summary="No adaptive hub reference changes were required", fix_type="verified")

    return FixResult(
        success=True,
        actions=actions,
        changes=changes,
        summary=f"Repaired stale adaptive hub references in {len(changes)} markdown file(s)",
        fix_type="code-fix",
    )
