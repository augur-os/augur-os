"""Shared test bootstrap for ingest skill tests.

Mirrors the root tests/conftest.py setup that migrated tests previously
inherited from their old central-tests/ home:
...
- add ingest skill scripts/ to sys.path so `from <module>` works for tests
  that load ingest internals
"""
...
# Make `skills.ingest.*` dotted imports resolvable.
...
_INGEST_SCRIPTS = Path(__file__).resolve().parents[2] / "scripts"
