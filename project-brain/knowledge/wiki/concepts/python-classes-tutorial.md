...
canonical_url: https://docs.python.org/3/tutorial/classes.html
content_hash: sha256:1b8164ed34ba3c3f96e964a8ef3fdadd6e5daafccccc77466d6833bc54061e82
tags:
- adr-753-real-data
- technical-tutorial
captured_at: '2026-05-16T00:44:37.048325Z'
...
- '[[wiki/concepts/python-aliasing]]'
- '[[wiki/concepts/python-inheritance]]'
- '[[wiki/concepts/python-scopes-and-namespaces]]'
- '[[wiki/languages/python]]'
- '[[wiki/technical-tutorial]]'
---


## Executive summary

- Python classes bundle data and behavior, creating a new type whose instances carry their own attributes and methods.
- Classes are first-class objects created at runtime and can be modified after creation, sharing in Python's dynamic nature.
- Name resolution follows the LEGB rule (local, enclosing, global, built-in); `global` and `nonlocal` let inner scopes rebind outer names.
- Method calls implicitly pass the instance as the first argument (`self`), which is declared explicitly in method definitions.
- Multiple base classes are supported; method lookup follows the MRO, allowing derived classes to override or extend base behavior.
- Built-in types may be used as base classes, and most operators with special syntax can be redefined for class instances.
- Aliasing — multiple names bound to the same mutable object — has pointer-like semantics that matter for arguments, attributes, and shared state.

## Key insights

1. A class statement creates a fresh local namespace; once the body finishes, that namespace becomes the class object's attribute dictionary, which is why class-level assignments become class attributes.
2. The distinction between class attributes and instance attributes is critical: mutable defaults stored on the class (e.g., a list) are shared by every instance, a common source of bugs.
3. Method binding is purely a runtime convention — `instance.method(args)` is equivalent to `Class.method(instance, args)`, which is why the first parameter is conventionally named `self` but is not a language keyword.
4. Python's namespace model treats `modname.funcname` as attribute access on the module object, unifying module globals and object attributes under one mechanism.
5. Inheritance is dynamic: built-in types can serve as bases, derived classes can override any method, and `super()` chains calls through the MRO without hardcoding parent names.

## Why it matters

For any Augur surface that touches Python source (extraction agents, ingest tools, the dashboard's Python backend), the official tutorial chapter on Classes is the canonical reference for scope, name resolution, and inheritance semantics — the very rules that determine whether a refactor preserves behavior. Linking this note as a [[technical-tutorial]] anchor gives future enrichment passes a stable target for "how does Python resolve this name?" questions, and ties into [[adr-753-real-data]] insofar as ADR-753's enrichment pipeline itself relies on correctly understanding mutable-default and aliasing semantics when threading note objects through the daemon.
...
> 9. Classes¶
> Classes provide a means of bundling data and functionality together. Creating a new class creates a new type of object, allowing new instances of that type to be made. Each class instance can have attributes attached to it for maintaining its state. Class instances can also have methods (defined by its class) for modifying its state.
> Compared with other programming languages, Python’s class mechanism adds classes with a minimum of new syntax and semantics. It is a mixture of the class mechanisms found in C++ and Modula-3. Python classes provide all the standard features of Object Oriented Programming: the class inheritance mechanism allows multiple base classes, a derived class can override any methods of its base class or classes, and a method can call the method of a base cla

## Source

- URL: https://docs.python.org/3/tutorial/classes.html
- Captured: 2026-05-16T00:44:37.048325Z
