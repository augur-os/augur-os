...
function ensureExternalDirs(name) {
  const home = os.homedir();
  const vaultDir = path.join(home, 'Vault', name);
  const documentsDir = path.join(home, 'Documents', name);
  const dirs = [
    vaultDir,
    path.join(vaultDir, 'skills'),
    path.join(vaultDir, 'drafts', 'staging'),
    documentsDir,
    path.join(documentsDir, 'adrs'),
  ];
  ...
  const marker = path.join(vaultDir, '.augur-vault');
  if (!fs.existsSync(marker)) {
    fs.writeFileSync(marker, `project: ${name}\n`, 'utf8');
  }
}
...
async function main() {
  ...
  if (name === '--help' || name === '-h') {
    ...
    console.log('  Promise: Get to know your AI setup, build your local second brain, and talk with your projects.');
    console.log('  Shell fallback for the fast-launch install prompt.');
    console.log('  Desktop chat first asks: Which folder should I initialize?');
    console.log('  After setup, run from the Augur repo: uv run aug init --project <folder>');
    console.log('  Next action: Ask Augur about this project.');
  ...
  const initResult = run('git', ['init', '-b', 'main'], { cwd: targetDir });
  ...
  ensureExternalDirs(name);
  success('Vault and documents directories created');
  console.log();
  ...
  console.log(`    cd ${name}`);
  console.log();
  console.log(bold('Fast launch next step:'));
  console.log('  Get to know your AI setup, build your local second brain, and talk with your projects.');
  console.log('  Which folder should I initialize?');
  console.log('  uv run aug init --project .');
  console.log(dim('  This creates or attaches project-brain/ and writes the read-only AI artifact inventory.'));
  console.log(dim('  Report the Browse URL: http://localhost:3000/browse'));
  console.log(dim('  Next action: Ask Augur about this project.'));
  ...
  console.log('    # Use the managed dev workflow when you need the dashboard.');
  console.log();
  console.log(dim('  This is the full repo-first shell fallback for contributors who want the complete workspace.'));
  console.log(dim('  Then add your documents and notes so Augur can start compounding.'));
  console.log();
  console.log(dim('  Docs:    https://augur.run/more.html'));
